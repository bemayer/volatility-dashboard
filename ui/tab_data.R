...
data_tab_ui < -  function(id) {
  ...
  tabItem(
          ...
          box(
            ...
            radioButtons(
              ...
              choices  =  list(
                ...
                "Squared Returns r_t²"  =  "squared",
                "Rolling RMS"  =  "rolling_rms",
            ...
            conditionalPanel(
              condition  =  paste0("input['", ns("target_measure"),
                                   "'] == 'rolling_rms' || input['",
                                   ns("target_measure"), "'] == 'rolling_std'"),
              numericInput(
                ns("rolling_window"),
                "Rolling window size:",
                value  =  20,
      ...
      fluidRow(
        box(
          ...
          div(
            ...
            p(paste0("Select a popular asset, enter a custom symbol, ",
    ...
    help_section(
      ...
      <ul>
        <li><strong>Popular Assets:< / strong> Click any button to quickly
        load common financial instruments< / li>
        <li><strong>Custom Symbol:< / strong> Enter any Yahoo Finance symbol
        (stocks, indices, currencies, commodities)< / li>
      ...
      <ul>
        <li><strong>Absolute Returns:< / strong> |r_t|  -  Simple absolute
        value of returns< / li>
        <li><strong>Squared Returns:< / strong> r_t²  -  Classical realized
        variance proxy< / li>
        <li><strong>Rolling RMS:< / strong> √(1 / n Σr²)  -  Root mean square
        over rolling window< / li>
        <li><strong>Rolling Std:< / strong> √(1 / (n - 1) Σ(r - r̄)²)  -
...
data_tab_server < -  function(input, output, session, values) {
  ...
  load_yahoo_data < -  function(symbol, name) {
    ...
    tryCatch({
      ...
      debug_log(paste("Data processed, returns:",
      ...
      removeModal()
      safe_notification(
        paste("Successfully loaded", name, "data with",
  ...
  observe({
    ...
    tryCatch({
      ...
      sample_data < -  read.csv(input$csv_file$datapath, nrows  =  5,
                                header  =  input$csv_header)
      col_names < -  names(sample_data)

      updateSelectInput(session, "date_column", choices  =  col_names,
                        selected  =  col_names[1])
      updateSelectInput(session, "price_column", choices  =  col_names,
  ...
  observeEvent(input$process_csv, {
    ...
    tryCatch({
      ...
      removeModal()
      safe_notification(
        paste("Successfully processed CSV with",
              length(processed_data$returns), "observations"),
        type  =  "message"
      )
  ...
  observeEvent(input$apply_config, {
    ...
    tryCatch({
      ...
      safe_notification(
        paste("Configuration error:", e$message),
        type  =  "error"
      )
    })
  })
  ...
  output$split_info < -  renderUI({
    ...
    div(
      ...
      p(paste("• Training set:", n_train,
              paste0("(", input$train_ratio, "%)"))),
      p(paste("• Test set:", n_test,
  ...
  output$price_plot < -  renderPlotly({
    ...
    p < -  plot_ly(plot_data, x  =  ~Date, y  =  ~Price,
