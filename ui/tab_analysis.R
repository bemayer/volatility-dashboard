    ...
    conditionalPanel(
      ...
      fluidRow(
        box(
          ...
          br(),
          div(
            style  =  paste0("background: #fff3cd; padding: 10px; ",
                             "border - radius: 5px;"),
      ...
      fluidRow(
        box(
          ...
          br(),
          div(
            style  =  paste0("background: #d1ecf1; padding: 10px; ",
                             "border - radius: 5px;"),
            p(strong("ARCH - LM Test:"),
              paste0("p - value < 0.05 suggests presence of ARCH effects ",
...
analysis_tab_server < -  function(input, output, session, values) {
  ...
  output$descriptive_stats < -  DT::renderDataTable({
    ...
    stats_df < -  data.frame(
      ...
      Mean  =  c(mean(returns, na.rm  =  TRUE),
                 mean(target_vol, na.rm  =  TRUE)),
      `Std Dev`  =  c(sd(returns, na.rm  =  TRUE),
                      sd(target_vol, na.rm  =  TRUE)),
      Minimum  =  c(min(returns, na.rm  =  TRUE),
                    min(target_vol, na.rm  =  TRUE)),
      Maximum  =  c(max(returns, na.rm  =  TRUE),
                    max(target_vol, na.rm  =  TRUE)),
      Skewness  =  c(moments::skewness(returns, na.rm  =  TRUE),
                     moments::skewness(target_vol, na.rm  =  TRUE)),
      Kurtosis  =  c(moments::kurtosis(returns, na.rm  =  TRUE),
  ...
  output$histogram_plot < -  renderPlotly({
    ...
    p < -  plot_ly(x  =  ~returns, type  =  "histogram", nbinsx  =  50,
  ...
  output$qq_plot < -  renderPlotly({
    ...
    p < -  plot_ly(plot_data, x  =  ~Theoretical, y  =  ~Sample,
                   type  =  "scatter", mode  =  "markers",
                   marker  =  list(color  =  "#e74c3c", size  =  4)) %>%
      add_trace(x  =  range(theoretical_quantiles),
  ...
  output$density_plot < -  renderPlotly({
    ...
    x_seq < -  seq(min(returns, na.rm  =  TRUE),
                   max(returns, na.rm  =  TRUE), length.out  =  100)
    normal_density < -  dnorm(x_seq, mean  =  mean(returns, na.rm  =  TRUE),
                              sd  =  sd(returns, na.rm  =  TRUE))

    p < -  plot_ly() %>%
      add_trace(x  =  density_est$x, y  =  density_est$y,
                type  =  "scatter", mode  =  "lines",
                name  =  "Empirical",
                line  =  list(color  =  "#3498db", width  =  3)) %>%
      add_trace(x  =  x_seq, y  =  normal_density,
                type  =  "scatter", mode  =  "lines",
                name  =  "Normal",
  ...
  create_acf_plot < -  function(series, title, max_lag  =  20) {
    ...
    p < -  plot_ly(plot_data, x  =  ~Lag, y  =  ~ACF, type  =  "bar",
      ...
      layout(
        ...
        shapes  =  list(
          ...
          list(type  =  "line", x0  =  min(plot_data$Lag),
  ...
  output$returns_timeseries < -  renderPlotly({
    ...
    p < -  plot_ly(plot_data, x  =  ~Date, y  =  ~Returns,
                   type  =  "scatter", mode  =  "lines",
  ...
  output$volatility_timeseries < -  renderPlotly({
    ...
    p < -  plot_ly(plot_data, x  =  ~Date, y  =  ~Volatility,
                   type  =  "scatter", mode  =  "lines",
  ...
  output$rolling_stats < -  renderPlotly({
    ...
    p < -  plot_ly() %>%
      add_trace(data  =  plot_data, x  =  ~Date, y  =  ~`Rolling Mean`,
                type  =  "scatter", mode  =  "lines",
                name  =  "Rolling Mean", line  =  list(color  =  "#3498db")) %>%
      add_trace(data  =  plot_data, x  =  ~Date, y  =  ~`Rolling Volatility`,
                type  =  "scatter", mode  =  "lines",
      ...
      layout(
        ...
        yaxis2  =  list(title  =  "Rolling Volatility", side  =  "right",
                        overlaying  =  "y"),
        hovermode  =  "x unified",
        legend  =  list(x  =  0.7, y  =  0.9)
      )
