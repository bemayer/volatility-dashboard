...
modeling_tab_server < -  function(input, output, session, values) {
  ...
  add_preset_models < -  function(preset_type) {
    ...
    if (preset_type == "basic") {
      new_models < -  list(
        ...
        list(family  =  "naive", type  =  "random_walk",
             name  =  "Random Walk"),
    ...
    DT::datatable(
      ...
      DT::formatStyle(
        ...
        backgroundColor  =  DT::styleInterval(
          cuts  =  quantile(
            metrics_df$RMSE, probs  =  c(0.33, 0.67), na.rm  =  TRUE
          ),
  ...
  output$dm_tests < -  DT::renderDataTable({
    ...
    tryCatch({
      ...
      dm_results < -  pairwise_dm_tests(test_target,
      ...
      if (!is.null(dm_results)) {
        DT::datatable(
          ...
          DT::formatRound(columns  =  seq_len(ncol(dm_results)),
                          digits  =  4) %>%
          DT::formatStyle(
