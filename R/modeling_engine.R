    ...
    if (window <= length(train_target)) {
      ...
      if (!is.null(test_target) && length(test_target) >= test_length) {
        ...
        for (h in 1:test_length) {
          ...
          forecasts[h] < -  mean(
            combined_data[start_idx:end_idx], na.rm  =  TRUE
          )
        }
      } else {
  ...
  for (window in windows) {
    if (window <= length(train_target)) {
      ...
      for (i in window:n_train) {
        ma_forecast < -  mean(
          train_target[(i  -  window   +  1):i], na.rm  =  TRUE
        )
        if (i < n_train) {
...
create_har_features < -  function(rv_series, daily_lag  =  1,
  ...
  for (i in weekly_window:n) {
    rv_weekly[i] < -  mean(
      rv_series[(i  -  weekly_window   +  1):i], na.rm  =  TRUE
    )
  }
  ...
  for (i in monthly_window:n) {
    rv_monthly[i] < -  mean(
      rv_series[(i  -  monthly_window   +  1):i], na.rm  =  TRUE
    )
  }
